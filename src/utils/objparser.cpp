...
void ObjParser::loadData(bool dataIsTriangles)
{
    ...
    std::vector<std::string> fileContents = utils::readTextFileAsLines(fileName);
    std::string line;
    ...
    for(unsigned int i = 0; i < fileContents.size(); i++)
    {
        ...
        {
            vertices.push_back(glm::vec3(utils::parseDouble(splitLine.at(1)),
                                    utils::parseDouble(splitLine.at(2)),
                                    utils::parseDouble(splitLine.at(3))));
        }
        else if (line.find("vn ") == 0)
        {
            normals.push_back(glm::vec3(utils::parseDouble(splitLine.at(1)),
                    utils::parseDouble(splitLine.at(2)),
                    utils::parseDouble(splitLine.at(3))));
        }
        else if (line.find("vt ") == 0)
        {
            textureCoords.push_back(
                glm::vec2(
                    utils::parseDouble(splitLine.at(1)),
                    utils::parseDouble(splitLine.at(2))
                )
            );
        }
        else if (line.find("f ") == 0)
        {
            if(dataIsTriangles)
            {
                std::vector<std::string> firstVert = utils::split(splitLine.at(1), '/');
                std::vector<std::string> secondVert = utils::split(splitLine.at(2), '/');
                std::vector<std::string> thirdVert = utils::split(splitLine.at(3), '/');
                faceVerts.push_back(glm::vec3(
					static_cast<int>(utils::parseDouble(firstVert.at(0))),
                    static_cast<int>(utils::parseDouble(secondVert.at(0))),
                    static_cast<int>(utils::parseDouble(thirdVert.at(0)))));
				faceTextures.push_back(glm::vec3(
					static_cast<int>(utils::parseDouble(firstVert.at(1))),
					static_cast<int>(utils::parseDouble(secondVert.at(1))),
					static_cast<int>(utils::parseDouble(thirdVert.at(1)))));
                if (firstVert.size() > 2)
                {
                    faceNormals.push_back(glm::vec3(
						static_cast<int>(utils::parseDouble(firstVert.at(2))),
						static_cast<int>(utils::parseDouble(secondVert.at(2))),
						static_cast<int>(utils::parseDouble(thirdVert.at(2)))));
                }
            }
            else /// Assumption: this must indicate quads because the graphics hardware doesn't really support anything else.
            {
				...
				std::vector<std::string> secondVert = utils::split(splitLine.at(2), '/');
				std::vector<std::string> thirdVert = utils::split(splitLine.at(3), '/');
				std::vector<std::string> fourthVert = utils::split(splitLine.at(4), '/');
                /// Make the first triangle
                faceVerts.push_back(glm::vec3(static_cast<int>(utils::parseDouble(firstVert.at(0))),
					static_cast<int>(utils::parseDouble(secondVert.at(0))),
					static_cast<int>(utils::parseDouble(fourthVert.at(0)))));
				faceTextures.push_back(glm::vec3(
					static_cast<int>(utils::parseDouble(firstVert.at(1))),
					static_cast<int>(utils::parseDouble(secondVert.at(1))),
					static_cast<int>(utils::parseDouble(fourthVert.at(1)))));
                if (firstVert.size() > 2)
                {
                    faceNormals.push_back(glm::vec3(
						static_cast<int>(utils::parseDouble(firstVert.at(2))),
						static_cast<int>(utils::parseDouble(secondVert.at(2))),
						static_cast<int>(utils::parseDouble(fourthVert.at(2)))));
                }
                /// Make the second triangle
				faceVerts.push_back(glm::vec3(static_cast<int>(utils::parseDouble(secondVert.at(0))),
					static_cast<int>(utils::parseDouble(thirdVert.at(0))),
					static_cast<int>(utils::parseDouble(fourthVert.at(0)))));
				faceTextures.push_back(glm::vec3(static_cast<int>(utils::parseDouble(secondVert.at(1))),
					static_cast<int>(utils::parseDouble(thirdVert.at(1))),
					static_cast<int>(utils::parseDouble(fourthVert.at(1)))));
                if (firstVert.size() > 2)
                {
					faceNormals.push_back(glm::vec3(static_cast<int>(utils::parseDouble(secondVert.at(2))),
						static_cast<int>(utils::parseDouble(thirdVert.at(2))),
						static_cast<int>(utils::parseDouble(fourthVert.at(2)))));
                }
            }
        ...
        else if(line.find("mtllib ") == 0)
        {
            ...
            std::vector<std::string> fileContents = utils::readTextFileAsLines(ss.str());
