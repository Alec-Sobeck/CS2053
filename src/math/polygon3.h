...
public:
	...
	/// throws std::invalid_argument if the Polygon3 being constructed would be degenerate or the
	...
	/// 
	/// Determines the point at which the ILineVariant intersects the polygon and returns the point, if one exists.
	/// \param line the ILineVariant to check
	/// \return A vec3 if an intersection exists, nullptr if one does not.
...
private:
	...
	/// 
	/// Creates the Vector describing the normal of the polygon. 
	/// 
	void computeNormal();
	/// 
